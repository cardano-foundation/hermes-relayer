...
On the Cosmos side, Cardano is tracked using explicit Cardano client types. The active probabilistic client type is `08-cardano-probabilistic`, with headers encoded as `/ibc.lightclients.probabilistic.v1.ProbabilisticHeader`. The deprecated Mithril-backed client type is `08-cardano-mithril`, with headers encoded as `/ibc.lightclients.mithril.v1.MithrilHeader`, and remains only for compatibility with historical deployments. Both authenticate a Cardano HostState update transaction so the verifier can extract the committed 32-byte `ibc_state_root` and store it in consensus state. Membership and non-membership then use standard ICS-23 proofs (protobuf `ibc.core.commitment.v1.MerkleProof` bytes) against that `ibc_state_root`.

`Height.revision_height` is a Cardano block number supplied by the Gateway's Yaci-backed bridge history, not a Cardano slot. After a Cardano transaction is included, Hermes waits until the Gateway reports an accepted light-client anchor at or after that block before it builds or relays proofs. In the active mode, acceptance is based on the configured `stake-weighted-stability` rules; it does not imply deterministic Cardano finality.
